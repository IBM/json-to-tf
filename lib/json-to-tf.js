...
function formatTfBlock(style, name, data) {
  let tf = "";
  if (
    contains(["terraform", "provider", "module", "output", "variable"], style)
  ) {
    tf += `${style}${style === "terraform" ? "" : ` "${name}"`} {`;
    tf += tfData(data, 0, style !== "variable");
    tf += `\n}`;
  } else {
    tf += `${style} ${name} {`;
    tf += tfData(data, 0);
...
function jsonToTf(jsonStr) {
  ...
  if (data.output) {
    eachKey(data.output, (output) => {
      tf += formatTfBlock("output", output, data.output[output]) + "\n\n";
    });
  }

  if (data.locals) {
    tf += `locals {`;
    tf += tfData(data.locals);
    tf += "\n}";
  }
